...
namespace ET
{
    public class DebugWindowLog : DebugWindowBase
    {
        ...
        protected override void OnDrawWindow(int id)
        {
            ...
            {
                var arr = ConsoleLogs.Instance.Logs.ToArray();
                for (int i = 0; i < arr.Length; i++)
                {
                    ConsoleLogs.LogInfo logInfo = arr[i];
                    ...
                    GUI.backgroundColor = logInfo.logType == LogType.Warning? Color.yellow
                    ...
                    var style = new GUIStyle("LargeButton");
                    style.alignment = TextAnchor.MiddleLeft;
                    var res = GUILayout.Toggle(this._selectedIndex == i,
                        $"[{logInfo.Hour:00}:{logInfo.Minute:00}:{logInfo.Second:00}]{titles[0]}", style, GUILayout.Width(this._windowRect.width));
            ...
            this._scrollPos2 = GUILayout.BeginScrollView(this._scrollPos2, GUILayout.Height(this._windowRect.height * 0.4f - 40));
            {
                ...
                {
                    var arr = ConsoleLogs.Instance.Logs.ToArray();
                    var log = arr[this._selectedIndex];
