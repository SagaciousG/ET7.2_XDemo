            ...
            else
            {
                ...
                if (path.EndsWith("Unity.Model.csproj"))
                {
                    ...
                    content = GenerateCustomProject(path, content,
                        ...
                        @"Assets\Scripts\Codes\Model\Generate\**\*.cs Generate\%(RecursiveDir)%(FileName)%(Extension)",
                        @"Assets\Scripts\Codes\Model\ConfigPartial\**\*.cs ConfigPartial\%(RecursiveDir)%(FileName)%(Extension)"
                        );
                }
