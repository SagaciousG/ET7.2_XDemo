...
namespace ET
{
    public class EventSystem: Singleton<EventSystem>, ISingletonUpdate, ISingletonLateUpdate
    {
        ...
        private class ActiveEventInfo
        {
            public ActiveEventInfo(IActiveEvent handler, Type component)
            {
                this.Handler = handler;
                this.Component = component;
            }

            public IActiveEvent Handler { get; }
        ...
        private MultiMap<int, Entity> allActives = new();

        private MultiMap<int, ActiveEventInfo> allActiveEvents = new();
        ...
        public void Add(Dictionary<string, Type> addTypes)
        {
            ...
            this.allActiveEvents.Clear();
            foreach (Type type in types[typeof (ActiveEventAttribute)])
            {
                object obj = Activator.CreateInstance(type);
                IActiveEvent activeEvent = obj as IActiveEvent;
                if (activeEvent == null)
                {
                    throw new Exception($"type mast implement from IActiveEvent. Name:{type.Name}");
                }
                
                object[] attrs = type.GetCustomAttributes(typeof(ActiveEventAttribute), false);
                foreach (object attr in attrs)
                {
                    var active = attr as ActiveEventAttribute;
                    this.allActiveEvents.Add(active.Key, new ActiveEventInfo(activeEvent, active.EntityType));
                }
            }
        }
        ...
        public void RegisterSystem(Entity component, bool isRegister = true)
        {
            ...
            var activeTypes = GetTypes(typeof(ActiveAttribute));
            if (activeTypes.Contains(type))
            {
                var attrs = type.GetCustomAttributes(typeof (ActiveAttribute), false);
                foreach (var attr in attrs)
                {
                    if (attr is ActiveAttribute active)
                    {
                        this.allActives.Add(active.Key, component);
                    }
                }
            }
        ...
        {
            if (this.allEntities.TryGetValue(instanceId, out var entity))
            {
                ...
                if (activeTypes.Contains(type))
                {
                    ...
                    foreach (var attr in attrs)
                    {
                        if (attr is ActiveAttribute active)
                        {
                            this.allActives.Remove(active.Key, entity);
                        }
                    }
                }
            }
        ...
        public void SetActive(int key, bool active, Scene scene = null)
        {
            if (this.allActives.TryGetValue(key, out var list))
            {
                foreach (Entity entity in list)
                {
                    if (scene == null || entity.DomainScene() == scene)
                    {
                        entity.ActiveSelf = active;
                        if (this.allActiveEvents.TryGetValue(key, out var handles))
                        {
                            foreach (ActiveEventInfo handle in handles)
                            {
                                if (entity.GetType() == handle.Component)
                                {
                                    handle.Handler.Run(entity, active);
                                }
                            }
                        }
                    }
