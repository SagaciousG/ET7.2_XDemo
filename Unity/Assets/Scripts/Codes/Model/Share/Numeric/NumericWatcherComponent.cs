...
namespace ET
{
    ...
    public static class NumericWatcherComponentSystem
    {
        ...
        public static void Run(this NumericWatcherComponent self, NumericComponent numericComponent, EventType.NumbericChange args)
        {
            ...
            SceneType unitDomainSceneType = numericComponent.DomainScene().SceneType;
            foreach (NumericWatcherInfo numericWatcher in list)
            {
                if (numericWatcher.SceneType != unitDomainSceneType && numericWatcher.SceneType != SceneType.None)
                {
                    continue;
                }
                numericWatcher.INumericWatcher.Run(numericComponent, args);
            }
        }
    }
