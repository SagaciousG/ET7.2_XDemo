...
{
    [Event(SceneType.Map)]
    public class ChangePosition_NotifyAOI: AEvent<ChangePosition>
    {
        protected override async ETTask Run(Scene scene, ChangePosition args)
        {
            ...
            var map = unit.DomainScene().GetComponent<MapComponent>().GetMap(unit.Map);
            map.GetComponent<AOIManagerComponent>().Move(aoiEntity, newCellX, newCellY);
