...
namespace ET.Server
{
    ...
    public class RealmHandler_Regist : AMRpcHandler<RegistRequest, RegistResponse>
    {
        protected override async ETTask Run(Session session, RegistRequest request, RegistResponse response, Action reply)
        {
            ...
            accountInfo.createTicks = TimeHelper.ServerNow();
            Log.Debug($"regist success {accountInfo}");
