    ...
    public class NetInnerComponentOnReadEvent: AEvent<NetInnerComponentOnRead>
    {
        ...
        protected override async ETTask Run(Scene scene, NetInnerComponentOnRead args)
        {
            try
            {
                ...
                OpcodeHelper.LogMsg(scene.DomainZone(), actorId, message, false, false, "InnerRead");
                // 收到actor消息,放入actor队列
                switch (message)
                {
                    ...
                    case IActorRequest iActorRequest:
                    {
                        ...
                        switch (mailBoxComponent.MailboxType)
                        {
                            ...
                            case MailboxType.UnitMessageDispatcher:
                            {
                                using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.Mailbox, realActorId))
                                {
                                    if (entity.InstanceId != realActorId)
                                    ...
                                    if (iActorRequest is IBattleRequest or IBattleMessage)
                                    {
                                        // var unitBattleComponent = entity.GetComponent<UnitBattleComponent>();
                                        // await ActorMessageDispatcherComponent.Instance.Handle(unitBattleComponent.BattleRoot, iActorRequest, Reply);
                                    }
                                    else
                                    {
                                        await ActorMessageDispatcherComponent.Instance.Handle(entity, iActorRequest, Reply);
