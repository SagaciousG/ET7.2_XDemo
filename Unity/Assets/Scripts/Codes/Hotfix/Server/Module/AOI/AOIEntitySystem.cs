    ...
    public static class AOIEntitySystem
    {
        ...
        public class AwakeSystem: AwakeSystem<AOIEntity, int, float3>
        {
            protected override void Awake(AOIEntity self, int distance, float3 pos)
            {
                ...
                var map = self.DomainScene().GetComponent<MapComponent>().GetMap(self.Unit.Map);
                map.GetComponent<AOIManagerComponent>().Add(self, pos.x, pos.z);
            }
        }
        ...
        public class DestroySystem: DestroySystem<AOIEntity>
        {
            ...
            {
                var map = self.DomainScene().GetComponent<MapComponent>().GetMap(self.Unit.Map);
                map.GetComponent<AOIManagerComponent>()?.Remove(self);
                self.ViewDistance = 0;
                self.SeeUnits.Clear();
        ...
        public static void EnterSight(this AOIEntity self, AOIEntity enter)
        {
            ...
            EventSystem.Instance.Publish(self.DomainScene(), new Event_UnitEnterSightRange() { A = self, B = enter });
        }
        ...
        public static void LeaveSight(this AOIEntity self, AOIEntity leave)
        {
            ...
            EventSystem.Instance.Publish(self.DomainScene(), new Event_UnitLeaveSightRange { A = self, B = leave });
        }
